...
#   complicated calculation this code must preform in processing a client's
#   request is a set intersection. This code is therefore significantly faster
#   overall and less data-instensive than version 1 (though not as fast as
#   version 2), and has much less time between a client's request and the
#   response than version two (though this isn't as quick as in version 1).
#
# Basic outline:
#
...
#   second-order friends respectively.
#
...
#   transaction's trustworthiness is than flagged accordingly.
#
# The merge(net,id_1,id_2) function is called whenever a new valid transaction
...
class Unique_id:
    ...
    def verification_4x(self,other,id_2):                                       # Checks if new request is from a fourth-order friend or lower
        if(id_2 in self.friends[0] or len(self.friends[1] & other.friends[1]) > 0):
            return "trusted"
        else:
            return "unverified"


# Function for updating friendships after a new transaction
...
with open(batch_in,'rU') as batch_file:                                         # Opening with 'rU' instead of 'r' circumvents a bug related to newline characters in csv module
    ...
    for row in batch_connections:
        ...
        # If a dictionary entry does not yet exist for one of the participants, create it and initialize its value as an empty instance of Unique_id
        ...
        # If they aren't already friends, update friends sets for new transaction    
